...
export async function GET(request: NextRequest) {
  ...
  const verifier = request.cookies.get("starter_oidc_verifier")?.value;
  if (!handoffCode || !verifier) {
    return NextResponse.redirect(
      new URL("/login?error=invalid_handoff", request.url),
    );
  }
  ...
  response.cookies.set("starter_oidc_verifier", "", {
    path: "/auth",
    maxAge: 0,
  });
