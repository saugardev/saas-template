...
Passwords use Argon2id. Verification and reset tokens are random, hashed at rest, expiring, and single-use. A successful password reset consumes all outstanding reset links and revokes existing sessions and OAuth refresh grants. Already issued access JWTs remain valid until their expiry (15 minutes by default, plus 30 seconds of clock tolerance).
